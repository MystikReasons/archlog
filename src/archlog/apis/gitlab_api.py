...
class GitLabAPI:
    ...
    def get(self, base_url: str, endpoint: str, params: Optional[Dict] = None) -> Optional[List[Dict]]:
    ...
    def get_commits_between_tags(
        ...
        :type tag_to: str
        :return: List of commit titles and their creation dates, or None on failure
        ...
        response = self.get(base_url, endpoint, params=params)
        if response:
            return [(commit.get("title", ""), commit.get("created_at", "")) for commit in response.get("commits", "")]
        else:
            return None
    ...
    def get_package_tags(self, base_url: str, project_path: str) -> Optional[List[Tuple[str, str]]]:
        ...
        response = self.get(base_url, endpoint)
        if response:
